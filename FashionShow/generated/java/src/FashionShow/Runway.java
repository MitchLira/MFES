...
public class Runway extends Event {
  ...
  public void addModels(final VDMSet newModels) {
    ...
    for (Iterator iterator_10 = newModels.iterator(); iterator_10.hasNext(); ) {
      Model m = (Model) iterator_10.next();
  ...
  public void remModels(final VDMSet oldModels) {
    ...
    for (Iterator iterator_11 = oldModels.iterator(); iterator_11.hasNext(); ) {
      Model model = (Model) iterator_11.next();
      models = SetUtil.diff(Utils.copy(models), SetUtil.set(model));
    }
  }
  ...
  public VDMSet getItemsInShow() {
    ...
    for (Iterator iterator_12 = MapUtil.rng(Utils.copy(expositionItems)).iterator();
        iterator_12.hasNext();
        ) {
      VDMSet item = (VDMSet) iterator_12.next();
